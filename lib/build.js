...
function build() {
    ...
    try {
        ...
        const linuiDir = new Set([...file_handle_1.readDirGetFile(`${config_1.MINI_PROGRAM_LIN_UI_DIR}/`)]);
        // 通过与组件存放目录差集获取所有未使用组件
        let differenceComponents = utils_1.difference(useComponents, linuiDir);
        ...
        const oldIgnore = new Set([...projectConfig.packOptions.ignore]);
        const resultIgnore = utils_1.deWeight(utils_1.union(oldIgnore, newIgnore), "value");
