...
function build() {
    if (!file_handle_1.checkFileExists('package.json')) {
        tip_style_1.Error(`${tip_style_1.error(`Can not find ${tip_style_1.warn('package.json')} file!`)} ${tip_style_1.error(`Please run ${tip_style_1.warn('npm init')} command!`)}`);
        return;
    }
    if (!file_handle_1.checkFileExists(config_1.NODE_MODULES_DIR_NAME)) {
        tip_style_1.Error(`${tip_style_1.error(`Can not find ${tip_style_1.warn(config_1.NODE_MODULES_DIR_NAME)} directory!`)} ${tip_style_1.error(`Please run ${tip_style_1.warn('npm install or yarn')}!`)} command`);
        return;
    }
